...
export function browserScript(since: Date): string {
  return `
...
const recent = convos.filter(c => c && c.uuid && (c.updated_at || c.created_at || '') >= SINCE);
const out = [];
for (const c of recent) {
  ...
  try {
    ...
    for (const m of (d.chat_messages || [])) {
      ...
      if (t && !/^<(command-|local-command|caveat)/.test(t)) { prompt = t.slice(0, 4000); break; }
    }
...
if (require.main === module) {
  ...
  process.stdout.write(browserScript(new Date(Date.now() - days * 86_400_000)) + "\n");
}
